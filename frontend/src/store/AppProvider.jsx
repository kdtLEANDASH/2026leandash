...
const getSettingsKey = (user) => {
  ...
  return `${SETTINGS_KEY_PREFIX}_${user.email || user.id}`;
};
...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    setCustomSettings(loadUserSettings(currentUser));
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      setCustomSettings(loadUserSettings(savedUser));
  ...
  const login = (email, password) => {
    ...
    if (user) {
      ...
      setCurrentUser(user);
      setCustomSettings(loadUserSettings(user));
      setIsAuthenticated(true);
    } else {
